...
from amaranth import Signal, Module, Elaboratable, Const, C, Cat
...
class UartTx(Elaboratable):
    ...
    """ This class implements the UART's tx. """
    ...
    def elaborate(self, platform):
        ...
        """ The function implements UART tx.  We use the sync domain
            for simulation and the slow domain for hardware. """

        start_value = self.freq_hz // self.baud_rate
        width = len(Const(start_value))

        print(f"UartTx: start_value = {start_value}, width = {width}")
        ...
        if platform is None:
            ...
            with m.If(ready | cnt[width]):
                # Cat needed ?
                m.d.slow += cnt.eq(start_value)
            with m.Else():
                m.d.slow += cnt.eq(cnt - 1)

            with m.If(cnt[width]):
                m.d.slow += data.eq(Cat(data[1:10], C(0, 1)))
            with m.Elif(valid & ready):
                m.d.slow += data.eq(Cat(C(0, 1), self.data, C(1, 1)))
