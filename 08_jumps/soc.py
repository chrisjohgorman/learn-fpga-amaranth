...
""" This module provides the system on chip (SOC) implementation
    for jump commands within the ALU and uses the riscv-assembly program
    to simplify the readability of the operations. """
...
class SOC(wiring.Component):
    ...
    """ This class describes the SOC arithmetic logic unit (ALU) encoding
        logic instructions with riscv assembler commands.  We create an
        infinate loop and test the implementation of jump commands. """
    ...
    def elaborate(self, platform):
        ...
        """ The arithmetic logic unit performs arithmetical and logic
            ...
            for further processing. To implement jumps we modify
            write_back_data and write_back_en variables, create a new
            variable next_pc and replace the pc+4 in the state machine
            with pc=next_pc. """
        ...
        # jump target
        next_pc = Mux(is_jal, pc + j_imm, Mux(is_jalr, rs1 + i_imm, pc + 4))
        ...
        # Register write back
        write_back_data = Mux((is_jal | is_jalr), (pc + 4), alu_out)
